...
public class RedisTest {
    ...
    //TODO:开始key失效的测试
    //TODO:小练习(未给出)：给出订单表-存在用户下的订单记录(状态：已支付、未支付)
    // --缓存中存储订单信息，设置过期时间如:10秒;设置定时器(10秒轮询一次)，找出已经不存在于缓存的订单记录---有可能超时了还未支付。
    @Test
    public void method6() throws InterruptedException {
        log.info("开始key失效的测试。。。。");
        final String key = "SpringBootRedis:Key:Expire";
        redisTemplate.delete(key);

        ValueOperations<String, String> valueOperations = redisTemplate.opsForValue();
        valueOperations.set(key, "peter", 5L, TimeUnit.SECONDS);

        log.info("第一次倒计时过程开始取出数据:{}", valueOperations.get(key));
        Thread.sleep(5000L);//执行后后睡眠5秒钟
        log.info("5秒后再取数据:{}", valueOperations.get(key));

        valueOperations.set(key,"coder-liu");//再给key设置value
        redisTemplate.expire(key,5L,TimeUnit.SECONDS);
        log.info("第二次倒计时过程开始取出数据:{}", valueOperations.get(key));
        Thread.sleep(5000L);//执行后后睡眠5秒钟
        log.info("5秒后再取数据:{}", valueOperations.get(key));
    }
